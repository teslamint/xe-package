<?php
    ...
    class planetController extends planet {
        ...
        function procPlanetContentWrite() {
            ...
            // twitter 연동 처리
            if(Context::get('twitter_autopush')=='Y') {
              $content = Context::get('content');
              $this->doPostToTwitter($myplanet->getTwitterUID(), $myplanet->getTwitterAuthKey(), $content);
            }
        ...
        function procPlanetTwitterApi()
        {
            $oPlanetModel = &getModel('planet');
            $myplanet = $oPlanetModel->getMemberPlanet();
            if(!$myplanet->isExists()) return new Object(-1,'msg_not_permitted');

            $args = Context::gets('twitter_uid','twitter_authkey','twitter_autopush');
            if(!$args->twitter_autopush) $args->twitter_autopush = 'N';

            $output = $this->doValidateTwitterInfo($args->twitter_uid, $args->twitter_authkey);
            if(!$output->toBool()) return $output;

            $args->module_srl = $myplanet->getModuleSrl();
            $args->member_srl = $myplanet->getMemberSrl();
            $output = executeQuery('planet.updatePlanetTwitter', $args);
            if(!$output->toBool()) return $output;

            $this->setMessage('msg_success_auth_twitter');
        }

        function doValidateTwitterInfo($user_id, $user_key) {
            require_once($this->module_path.'libs/twitter.api.php');
            $oTwit = new Twitter($user_id, $user_key);
            $output = $oTwit->authenticate();
            if ($output != FALSE) return new Object();
            return new Object(-1, 'twitter_auth_failed');
        }

        function doPostToTwitter($user_id, $user_key, $body) {
            require_once($this->module_path.'libs/twitter.api.php');
            $oTwit = new Twitter($user_id, $user_key);
            if ($oTwit->send($body) != FALSE) return new Object();
            return new Object(-1, 'twitter_post_failed');
        }
